...
public class BankLayoutTest
{
	...
	@Mock
	@Bind
	private ZigzagBankTagTabLayoutConfig config;
	...
	@Inject
	private ZigzagBankTagTabLayoutPlugin plugin;
...
//            System.out.println("tostring: " + id + " " + item + " " + type);
	...
	private LaidOutBank layOutBank(Layout layout, List<Widget> bankItems)
	{
		...
		Set<ZigzagBankTagTabLayoutPlugin.FakeItem> fakeItems = plugin.calculateFakeItems(layout, indexToWidget);

		for (ZigzagBankTagTabLayoutPlugin.FakeItem fakeItem : fakeItems)
		{
	...
	public void testLayoutGeneratorWithDuplicateItems() {
		...
		Layout layout = layoutGenerator.generateLayout(Arrays.asList(GAMES_NECKLACE_8.id, RUNE_PLATEBODY.id), Arrays.asList(GAMES_NECKLACE_8.id, MAGIC_LOGS.id, MAGIC_LOGS.id), Collections.emptyList(), Collections.emptyList(), Layout.emptyLayout(), 28);
	...
	public void testLayoutGeneratorWithDuplicateLimit() {
		...
		Layout layout = layoutGenerator.generateLayout(Collections.emptyList(), Arrays.asList(
			...
			MAGIC_LOGS.id,
			MAGIC_LOGS.id,
			GAMES_NECKLACE_8.id),Collections.emptyList(), Collections.emptyList(), Layout.emptyLayout(), 3);
		checkLayout(layout, 0, GAMES_NECKLACE_8);
	...
	public void testLayoutGeneratorWithMoreItemsThanDuplicateLimitAsTheFirstItemsInTheInventory() {
		...
		Layout layout = layoutGenerator.generateLayout(Collections.emptyList(), Arrays.asList(
			...
			MAGIC_LOGS.id,
			MAGIC_LOGS.id
		), Collections.emptyList(), Collections.emptyList(), Layout.emptyLayout(), 3);
	...
	public void testLayoutGeneratorWithEmptyInventory() {
		...
		Layout layout = layoutGenerator.generateLayout(Collections.emptyList(), Arrays.asList(),Collections.emptyList(), Collections.emptyList(), Layout.emptyLayout(), 3);
		assertEquals(0, layout.allPairs().size());

		layout = layoutGenerator.generateLayout(Collections.emptyList(), Arrays.asList(-1),Collections.emptyList(), Collections.emptyList(), Layout.emptyLayout(), 3);
		assertEquals(0, layout.allPairs().size());

		layout = layoutGenerator.generateLayout(Collections.emptyList(), Collections.nCopies(28, -1),Collections.emptyList(), Collections.emptyList(), Layout.emptyLayout(), 3);
		assertEquals(0, layout.allPairs().size());
	}
	...
	public void testLayoutGeneratorDoesntDeleteDuplicateItems() {
		...
		Layout layout = layoutGenerator.generateLayout(Collections.emptyList(), Arrays.asList(
			...
			MAGIC_LOGS.id,
			MAGIC_LOGS.id
		),Collections.emptyList(), Collections.emptyList(), initialLayout, 3);
		checkLayout(layout, 0, MAGIC_LOGS);
		checkLayout(layout, 8, MAGIC_LOGS);
		checkLayout(layout, 1, MAGIC_LOGS);
